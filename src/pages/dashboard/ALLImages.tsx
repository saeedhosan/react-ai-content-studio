...
function ImageGenerator() {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        if (!prompt) {
            toast.warn("Please shorly tell us about the image you want to generate!");
            return false;
        }
        ...
        //only for demo
        // axios.post(endpoints.images.create, {
        axios
            .get(endpoints.images.create, {
                data: {
                    prompt,
                    size: size.value,
                    title,
                    ...
                    <form onSubmit={handleSubmit}>
                        <div className="row">
                            <div className="col-sm-12">
                                <div
                                    className={`text-left mb-4 ${borderRound("round-full")}`}
                                    id="balance-status"
                                >
...
function Showimges() {
    const { images, refetch: refetchImages } = useImages();
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    ...
    useMemo(() => {
        ...
        let fileterd = null;
        fileterd = images?.filter((item: ImageResponseType) => {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                (async () => {
                    try {
                        //only for demo
                        const resp = await axios.get(endpoints.images.delete + id);
    ...
    const columns: any = [
        {
            ...
            sortable: true,
            selector: (image: ImageResponseType) => {
                return (
        ...
        },
        {
            ...
            sortable: true,
            selector: (image: ImageResponseType) => <span>{image.size}</span>,
        },
        {
            name: <b className="text-uppercase">created_at</b>,
            selector: (row: ImageResponseType) => {
                return (
                    <>
        ...
        },
        {
            ...
            selector: (image: ImageResponseType) => (
                <div>
                    <a download={image.url} target="_blank" href={image.url || ""}>
                        <i
                            className="fa-solid fa-cloud-arrow-down icon-btn text-success"
                        ...
                        <div className="dataTables_wrapper">
                            <div className="col-md-9 float-left">
                                <h3 className="card-title fs-16 mt-3 mb-4">
                                    <i className="fa-solid fa-image-landscape mr-4 green-icon fs-14" />
                                    Generated Images
                                </h3>
                            </div>
